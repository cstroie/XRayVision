...
def db_add_exam(uid, metadata, report):
    ...
    if report:
        ...
        poz = 0
        status = 'queued'
    # Get the analyzed timestamp (now)
    ...
    with sqlite3.connect(DB_FILE) as conn:
        conn.execute('''
            INSERT OR REPLACE INTO exams (uid, name, id, age, created, protocol, region, analyzed, report, positive, iswrong, reviewed, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE((SELECT iswrong FROM exams WHERE uid = ?), 0), ?, ?)
        ''', (uid, metadata["patient"]["name"], metadata["patient"]["id"], metadata["patient"]["age"], created, metadata["exam"]["desc"], metadata['exam']['anatomy'], now, report, poz, uid, 0, status))
    # If new exam is queued, notify the event
    if status == 'queued':
        queue_event.set()
